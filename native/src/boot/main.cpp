...
Supported actions:
  ...
  ramdisk-table <file> [commands...]
    ...
    Supported commands:
      print
        Prints the table
      rm --name <name> OR --entry <entry_num>
        Removes table entry with name <name> or entry number <entry_num>, if present
      add --name <name> --type <type> [--id <0..15> <hex_val>] [--copy-id <entry>]
        Add new table entry with name <name> and type of <type>.
        A ramdisk file with name <name>.cpio has to be present
        in the vendor ramdisk directory.
        <name> has to be shorter than 31 characters.
